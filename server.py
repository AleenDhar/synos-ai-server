    ...
    async def initialize_agent(
        ...
        if enabled_mcp_servers:
            try:
                ...
                print(f"Loaded {len(mcp_tools)} MCP tools from {len(enabled_mcp_servers)} servers")
...
Be specific about what needs improvement."""
        ...
        if instructions is None:
            instructions = """You are an expert AI assistant with access to web search and various tools.

CAPABILITIES:
- Web search using DuckDuckGo (free, no API key needed)
- Deep research using specialized research agents
- File operations for managing large content
...
Use files to manage large amounts of information.
Be thorough but concise in your final answers."""
        ...
        self.agent = create_deep_agent(
            tools=tools,
            system_prompt=instructions,  # Changed from 'instructions' to 'system_prompt'
            subagents=subagents,
            model=config.MODEL,
